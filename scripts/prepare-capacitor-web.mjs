...
lines.push(
  ...
  "    <script>",
  "      // Native splash uses launchAutoHide:false; React hides it after first",
  "      // paint. These fallbacks guarantee it can never cover an error screen",
  "      // or stay up forever if the bundle fails to load.",
  "      function netroHideSplash() {",
  "        try {",
  "          var C = window.Capacitor;",
  "          if (C && C.Plugins && C.Plugins.SplashScreen) C.Plugins.SplashScreen.hide();",
  "        } catch (err) { /* no native splash */ }",
  "      }",
  "      setTimeout(netroHideSplash, 8000);",
