...
import {
  User, ShieldCheck, Bell, Heart, CreditCard, Languages, Palette, HelpCircle,
...
function Account() {
  ...
  const groups: { title: string; items: { icon: typeof User; label: string; to: string; sub?: string }[] }[] = [
    ...
    },
    {
      title: "Money",
      items: [
        { icon: Wallet, label: t("wallet"), to: "/wallet", sub: "SAR 120.00" },
        { icon: CreditCard, label: t("paymentMethods"), to: "/account/payment" },
        { icon: Ticket, label: t("promotions"), to: "/account/promotions" },
      ],
    },
    {
      title: t("appearance"),
      items: [
        { icon: Languages, label: t("language"), to: "/account/language", sub: locale === "ar" ? "العربية" : "English" },
      ...
      <ScreenBody>
        ...
        {groups.map((g) => (
          <section key={g.title} className="mt-6">
            <h3 className="mb-2 px-2 text-xs font-semibold uppercase tracking-widest text-muted-foreground">{g.title}</h3>
        ...
        <p className="mt-6 text-center text-[11px] text-muted-foreground">NETRO v1.0 · Made with ⚡</p>
      </ScreenBody>
      <BottomNav />
    </MobileScreen>
  );
