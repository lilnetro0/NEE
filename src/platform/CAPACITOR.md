...
`webDir/index.html`. Official `spa.enabled` prerender also fails with this Nitro
layout (`dist/server/server.js` missing).

This repo uses:

1. `npm run build` → `.output/public` + `.output/server`
2. `npm run prepare:capacitor` → writes `.output/public/index.html` with:
   - visible boot UI (never a blank black screen)
   - `window.$_TSR` SPA shell bootstrap (required by Start `hydrateStart`)
   - hashed client entry from `src/client.tsx`
3. `capacitor.config.ts` → `webDir: ".output/public"`

**Black-screen root cause (fixed):** loading `index-*.js` into an empty body without
`$_TSR.router` made production `hydrate()` throw `Invariant failed` before React
could render. The custom client + prepare script now inject that shell and paint
boot errors inline.
